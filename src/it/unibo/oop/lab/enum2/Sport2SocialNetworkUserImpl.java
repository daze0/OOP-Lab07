...
public class Sport2SocialNetworkUserImpl<U extends User> extends SocialNetworkUserImpl<U> {
    ...
    public Set<Sport> getIndividualSports() {
    	...
    	return Set.copyOf(individualSports);
    }
    ...
    public Set<Sport> getSportPracticedInPlace(Place p) { 
    	Set<Sport> sportsInGivenPlace = new HashSet<>();
    	for(Sport s : this.sports) {
    		if(s.getPlace() == p) {		//it is possible to use == instead of equals() since we're comparing two enum constants
    			sportsInGivenPlace.add(s);
    		}
    	}
    	return Set.copyOf(sportsInGivenPlace); 
    }
}
